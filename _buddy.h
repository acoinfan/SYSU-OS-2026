...
// Buddy节点总数
#define MAX_TOTAL_BLOCKS 1024
// 最大的Buddy节点可以包含的块数为 2 ^ (MAX_ORDER) = 1024
#define MAX_ORDER 10
...
class Buddy {
public:
    void initialize(char* _bitmap, int _totalPages);
...
public:
    // 1为被占用 0为未占用
    char freeBitmap[MAX_TOTAL_BLOCKS / 8 + 1]; // 管理freeNodes的BitMap
    FreeNode freeNodes[MAX_TOTAL_BLOCKS];  // 静态数组
